...
class respartner_inherit123(models.Model):
    _inherit = "res.partner"

    bq_code = fields.Char(string='BQ Code')
...
##################### ค้นหาฟิวในช่อง Many2one // Form sale order partner_shipping_id #####################################
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        if operator not in ('ilike', 'like', '=', '=like', '=ilike'):
            return super(respartner_inherit123, self).name_search(name, args, operator, limit)
        args = args or []
        # print(operator)
        domain = ['|', ('ref', operator, name), ('name', operator, name)]
        partners = self.env['res.partner'].search(['|',
                                                   ('name', operator, name),
                                                   ('bq_code', operator, name)], limit=limit)
        print(partners.ids)
        if partners:
