...
class dev_export_wizard(models.TransientModel):
    ...
    def export_excel(self):
        ...
        if not rec.split_sheet or not rec.relational_field:
            ...
            obj_len = int(len(obj_ids))
            for l in range(0,obj_len):
                worksheet.append(l)
        ...
        if rec.is_group_by:
            ...
            col=0
            l_group_field_label = int(len(group_field_label))
            for i in range(0, l_group_field_label):
            ...
            for result in results:
                ...
                if isinstance(result[rec.group_by_field_id.name], tuple):
                    worksheet[0].merge_range(row,col,row,int(las_col), result[rec.group_by_field_id.name][1] or ' ',group_format)
                else:
                    worksheet[0].merge_range(row,col,row,int(las_col), result[rec.group_by_field_id.name] or ' ',group_format)
                row+=1
                if rec.sub_group_by_field_id:
                    ...
                    for sub_result in sub_results:
                        ...
                        if isinstance(sub_result[rec.sub_group_by_field_id.name], tuple):
                            worksheet[0].merge_range(row,col,row,int(las_col), sub_result[rec.sub_group_by_field_id.name][1] or ' ',group_format)
                        else:
                            worksheet[0].merge_range(row,col,row,int(las_col), sub_result[rec.sub_group_by_field_id.name] or ' ',group_format)
                        if rec.third_group_by_field_id:
                            ...
                            for third_result in third_results:
                                ...
                                if isinstance(third_result[rec.third_group_by_field_id.name], tuple):
                                    worksheet[0].merge_range(row,col,row,int(las_col), third_result[rec.third_group_by_field_id.name][1] or ' ',group_format)
                                else:
                                    worksheet[0].merge_range(row,col,row,int(las_col), third_result[rec.third_group_by_field_id.name] or ' ',group_format)
                                group_format = workbook.add_format()
                                group_format = self.set_group_format(rec,group_format)
                                row+=1
                                for val in third_result['values']:
                                    ...
                                    row+=1
                                if rec.is_sum:
                                    for ran in range(0,int(len(group_field_label))):
            ...
            for model_id in obj_ids:
                ...
                if rec.template == 'template1' and not rec.relational_field:
                    if label_count == 0:
                        col=0
                        for i in range(0,int(len(main_field_label))):
                            worksheet[sheet_i].write(row,col, main_field_label[i] or ' ',main_label_format)
                            col+=1
                        row+=1
                        label_count +=1
                    col=0
                    for i in range(0,int(len(main_vals))):
                    ...
                    for i in range(0,int(half)):
                        ...
                        count+=1
                        my_row+=1
                    for i in range(half,int(len(main_field_label))):
                        col=4
                ...
                if rec.relational_field and rec.relational_fields_ids:
                    ...
                    col+=1
                    for i in range(0,int(len(line_field_label))):
                    ...
                    for line in line_ids:
                        ...
                        col+=1
                        for i in range(0,int(len(line_main_val))):
                        ...
                        row+=1
                    if rec.is_sum:
                        for sum_i in range(0,int(len(line_main_val))):
            ...
            if rec.template == 'template1' and not rec.relational_field and rec.is_sum:
                for sum_i in range(0,int(len(main_vals))):
