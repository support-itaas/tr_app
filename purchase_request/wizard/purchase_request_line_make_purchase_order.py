...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    @api.model
    def _prepare_purchase_order(self, picking_type, group_id, company, origin, purchase_type, order_type, matrix_id, assigned_to):
    ...
    def _prepare_purchase_order_line(self, po, item):
        ...
        vals = {
            ...
            'product_uom': item.line_id.product_uom_id.id,
            'price_unit': 0.0,
            'product_qty': qty,
        ...
        if vals:
            vals.update({'product_uom': item.line_id.product_uom_id.id,
                         'price_unit': item.line_id.estimated_cost, })
        # print("vals : " + str(vals))
        return vals
    ...
    def make_purchase_order(self):
        ...
        if check_order_type:
            ...
            for item in self.item_ids:
                ...
                if not purchase:
                    matrix_id = self.env['purchase.approval.matrix'].search([('purchase_type','=',line.request_id.purchase_type.id), ('type','=','PO')], limit=1)
                ...
                if available_po_lines and not item.keep_description:
                    ...
                    # print('po_line_data : ' + str(po_line_data) )
                    if item.keep_description:
                        po_line_data['name'] = item.name
                    if not item.keep_unit_price:
                        po_line_data['price_unit'] = 0.0
