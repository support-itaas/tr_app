...
class HrSalaryRule(models.Model):
    ...
    @api.multi
    def cal_sum_benefits(self, payslip, code):
        res = 0
        payslip_id = self.env['hr.payslip'].search([('id','=',payslip)])
        if payslip_id.benefit_line_ids:
            for ben in payslip_id.benefit_line_ids:
                if ben.category_id.code == code:
                    res += ben.employee_amount
        return res
