...
class JournalLedgerReportWizard(models.TransientModel):
    ...
    def onchange_type(self):
        print ('---Print type')
        journal_list_ids = self.env['account.journal'].search([('type','=',self.type)])
        print ('journal_list_ids',journal_list_ids)
        self.journal_ids = journal_list_ids
        # self.journal_ids = journal_ids
        # self.write({'journal_ids': (6, 0, [journal_list_ids.ids])})
