...
class StockReportXls(models.AbstractModel):
    ...
    def get_lines(self, data, warehouse, c_from, c_to):
        lines = []
        categ_id = data.mapped('id')
        if categ_id and c_from and c_to:
            stock_history = self.env['product.product'].search(
                [('default_code', '>=', c_from), ('default_code', '<=', c_to), ('categ_id', 'in', categ_id)])
        elif categ_id:
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for i in get_warehouse[1]:
            get_line = self.get_lines(d, i, c_from, c_to)
