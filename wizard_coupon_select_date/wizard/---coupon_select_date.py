...
class Wizard_Select_Date(models.TransientModel):
    ...
    def print_report(self):
        ...
        if self.is_detail:
            ...
            if self.coupon_id:
                ...
                raise UserError(_('เลือกคูปอง หรือ เลือกรหัสบัญชี และ วันที่ ก่อนเรียกรายงาน11111'))
        ...
        #
                ...
                # worksheet.write(inv_row, 12, sum_amount, for_center_bold)
                # worksheet.write(inv_row, 13, end_balance, for_center_bold)
                for coupon_id in data_temp:
                    ...
                    if self.is_gl_balance:
                        worksheet.write(inv_row, 7, coupon_id.amount * (end_balance/sum_amount), for_right)
                    else:
    ...
    def coupon_and_gl_movement(self):
        ...
        if self.date:
            # ----------------HEADDER-------------------------------------
            worksheet = workbook.add_sheet('Coupon and GL Movement')
            worksheet.col(0).width = 4000
            worksheet.col(1).width = 10000
            worksheet.col(2).width = 3000
            worksheet.col(3).width = 4500
            ...
            if session_with_coupon_ids:
                for session_id in session_with_coupon_ids:
                    ...
                    reference = ""
                    if self.account_id:
                        account_ids = session_id.mapped('order_ids').filtered(lambda o: o.amount_total > 0).mapped('account_move').mapped('line_ids').filtered(
                            lambda o: o.account_id == self.account_id).mapped('account_id')

                        if not account_ids:
                            account_ids = session_id.mapped('order_ids').filtered(lambda o: o.amount_total > 0).mapped('invoice_id').mapped('move_id').mapped('line_ids').filtered(lambda o: o.account_id == self.account_id).mapped('account_id')
                    ...
                    account_move_ids = session_id.mapped('order_ids').filtered(lambda o: o.amount_total > 0).mapped('account_move')
        ...
        #########new work sheet#########
        if self.is_detail:
            worksheet = workbook.add_sheet('Coupon Balance ยกมา')
            ...
            worksheet = workbook.add_sheet('Coupon Balance ยกไป')
            # worksheet.col(0).width = 4000
            # worksheet.col(1).width = 10000
            # worksheet.col(2).width = 3000
            # worksheet.col(3).width = 4500
        ...
        if self.is_use_detail:
            worksheet = workbook.add_sheet('Coupon ใช้ไปทั้งหมด')
            inv_row = 1
            # date_from = strToDate(self.date_from).strftime("%d/%m/%Y")
            at_date = strToDate(self.date).strftime("%d/%m/%Y")
