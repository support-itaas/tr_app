    ...
    def _check_company_operating_unit(self):
        for rec in self:
            if (rec.company_id and rec.operating_unit_id and
                ...
                raise ValidationError(_('Configuration error. The Company "%s" in'
                                        ' the Sales Order and in the Operating "%s" '
                                        ' Unit must be the same.') % (rec.company_id.name, rec.operating_unit_id.name))
